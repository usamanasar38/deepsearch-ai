...
import { z } from "zod";
import { evalsModel } from "@/server/ai/model";
...
export const checkRelevancy = async (opts: {
  question: string;
  submission: string;
}) => {
  ...
  // If there are no statements, return a score of 0
  if (statementsObject.statements.length === 0) {
    return {
      score: 0,
      metadata: {
  ...
  // Calculate the score based on the verdicts
  const scores = {
    yes: 1,
    no: 0,
    unsure: 0.5,
  };
  const totalScore = verdictsObject.verdicts.reduce(
